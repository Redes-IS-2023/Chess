...
bool valMoverPeon(char piezaAct, char piezaEnPos, int posAct, int pos){
	if (isupper(piezaAct)){	// Se moveria para abajo por ser blanca
		if (posAct + 10 == pos){ // Movimiento vertical si no hay pieza en frente
			if (piezaEnPos == ' ')
				return true;
			return false;
		}
		if (posAct + 9 == pos || posAct + 11 == pos){ // Movimiento en diagonal hacia el frente solo si puede comer pieza enemiga
			if (piezaEnPos != ' ')	// Ya se valido antes que no sea una pieza del mismo jugador
				return true;
			return false;
		}
	} else {	// Se moveria hacia arriba por ser negra
		if (pos + 10 == posAct){
			if (piezaEnPos == ' ')
				return true;
			return false;
